    ...
    >
      <div className="space-y-8 mt-2" ref={containerRef}>
        {skills.map((s, i) => (
          ...
          >
            <div className="relative flex justify-between items-center gap-8 p-4 overflow-hidden rounded-full font-normal sm:text-3xl">
              <div
                className="absolute h-full bg-(--bg-primary-inverse) left-0 top-0 progress origin-left rounded-full"
                style={{ width: `${s.progress}%` }}
              />

              <div className="flex gap-4 z-1 mix-blend-difference ">
                <CountTitle
                  idx={i}
